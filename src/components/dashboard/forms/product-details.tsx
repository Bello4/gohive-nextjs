...
import { OfferTag } from "@/types/offerTag";
...
const ProductDetails: FC<ProductDetailsProps> = ({
  ...
  useEffect(() => {
    const getSubCategories = async () => {
      const res = await getAllCategories(form.watch().categoryId);
  ...
  const handleSubmit = async (values: z.infer<typeof ProductFormSchema>) => {
    try {
      ...
      const response = await upsertProduct(
        {
          productId: data?.productId, // include only if editing
        ...
        <CardContent>
          <Form {...form}>
            ...
            >
              ...
              {!isNewVariantPage && (
                <InputFieldset
                  label="Free Delivery For Customers (Optional)"
                  description="Free around town ?"
                >
                  <div>
                    <label
                  ...
                  <div>
                    <p className="mt-4 text-sm text-main-secondary dark:text-gray-400 pb-3 flex">
                      <Dot className="-me-1" />
                      If not select the places you want to deliver this product
                      to for free
                    </p>
                  </div>
                  <div className="">
                    {!form.getValues().freeShippingForAllCountries && (
                      <div>
                        ...
                        <p className="mt-4 text-sm text-main-secondary dark:text-gray-400 pb-3 flex">
                          <Dot className="-me-1" />
                          List of places you offer free delivery for this
                          product :&nbsp;
